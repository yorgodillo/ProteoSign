...
var uploadFiles = function (serverSide, postSuccess) {
   ...
   uploaded_files = uploaded_files + nToUpload;
   $("#s2btnf").prop('disabled', true);
...
var postFile = function (idx, file, serverSide, postSuccess) {
   ...
   var progresstditm = $("#s2uluploaders table tr:nth-child(" + (uploaded_files + idx) + ") td").get(1);
         ...
         uploadingFiles = this.files;
			...
			if(oversized_files_idxs.length == 0)
			{
		      uploadFiles(false, function () {
				 ...
				 if (Files_valid == true && (upload_was_successful == true || (++nUploaded == uploaded_files && (peptideLabelsNamesFromFile.length > 0 || peptideLabelsFromFile.length > 0)))) {
					upload_was_successful = true;
		            $("#s2btnf").prop('disabled', false);
					console.log("button enadbled, nuploaded/ntoupload" + nUploaded + "/" + nToUpload);
		         }
		      });			
			}
			else
