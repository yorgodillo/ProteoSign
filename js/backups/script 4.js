...
var uploadFiles = function (serverSide, postSuccess) {
   ...
   $.each(uploadingFiles, function (idx, file_i) {
	  ...
	  console.log("	created table row for file: " + idx + file_i.toString());
   });
   // fire AJAX calls
   $.each(uploadingFiles, function (idx, file_i)
   {
      postFile(idx, file_i, serverSide, postSuccess);
	  console.log("	fired post file for file: " + idx);
   });
  }

// Called by respective AJAX event handlers (see postFile function below)
var uploadFinished = function (success, idx, file) {
	console.log("	called for idx: " + idx + "nToUpload: " + nToUpload);
	nToUpload = nToUpload - 1;
	if(nToUpload == 0)
	{
		uploaded_files = uploaded_files + tmp_nToUpload;
...
var postFile = function (idx, file, serverSide, postSuccess) {
   ...
   var progresstditm = $("#s2uluploaders table tr:nth-child(" + (uploaded_files + idx + 1) + ") td").get(1);
   console.log("	current uploaded files / idx in postfile " + uploaded_files + "/" + idx);
   $.ajax({
            ...
            myXhr.upload.addEventListener('progress', function (e) {
               if (e.lengthComputable) {
                  if (e.loaded / e.total == 1.0) {
                     ...
                     helper_setItemAttr("#uploadfile" + (uploaded_files + idx), {value: e.loaded, max: e.total});
						//console.log("uploaded files " + uploaded_files + " idx: " + idx);
                  }
               }
      ...
      beforeSend: function (jqXHR, settings) {
         if (serverSide) {
            helper_setItemAttr("#uploadfile" + (uploaded_files + idx), {value: 0, max: 100});
         }
      }}).done(function (data, textStatus, jqXHR) {
      	if(sectionSpinnerOn)
	      	toggleCurrentSectionSpinner();
...
//If server-side everything went fine (internal things that the server had to do with the client's file, such as storage etc)
      uploadFinished(data.success, (uploaded_files + idx), file);
...
//label-free case (disable non-applicable parameters and rename others accordingly)
      ...
      if (typeof (postSuccess) == "function") {
		  console.log("entered post success lines");
         postSuccess();
      }
   }).fail(function (jqXHR, textStatus, errorThrown) {
   	if(sectionSpinnerOn)
      ...
      uploadFinished(false, (uploaded_files + idx), file);
   });
}
