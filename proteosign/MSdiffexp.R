...
do_results_plots<-function(norm.median.intensities,time.point,exportFormat="pdf",outputFigsPrefix=""){
  ...
  write.table(diffexp[signTruth,],file=paste(outputFigsPrefix,"_diffexp_",time.point,".txt",sep=""),sep="\t",row.names=F,quote=F)
  ...
  write.table(diffexp,file=paste(outputFigsPrefix,"_results_",time.point,".txt",sep=""),sep="\t",row.names=F,quote=F)
...
do_limma_analysis<-function(working_pgroups,time.point,exp_design_fname,rep_structure,exportFormat="pdf",outputFigsPrefix=""){
  ...
  if(n_techreps > 1){	# if we have technical replicates (duplicated elements in rep_structure)
...
#################################################################################################################################
##
## PATCHED -- number of conditions/labels-independent function
##
#################################################################################################################################
    ...
    if(length(light_mod_index) == 1){
	  ...
	  for(cond_i in heavier.labels.Modifications){
	    ...
	    for(label_i in unlist(cond_i)){
	      labelTruth<-labelTruth | grepl(label_i,x$Modifications)
	    }
...
addLabel<-function(lblname, lbl.Modifications){
  ...
  if(length(lblname_i) != 0){
    cat(paste("addLabel: Error adding label '",lblname,"': An existing label with name '",lblname,"' (specification: ",paste(unlist(conditions.labels.Modifications[lblname_i]),collapse=", "),") already exists. Please try a different name.",sep=""))
    return(FALSE)
  }
  i<-1
